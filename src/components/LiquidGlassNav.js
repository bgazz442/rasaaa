...
// Ultra Premium iOS Spring - Heavy & Smooth
const liquidSpring = {
  type: 'spring',
  stiffness: 220,
  damping: 28,
...
// Quick response for UI elements
const quickSpring = {
  type: 'spring',
  stiffness: 350,
  damping: 32,
...
// Liquid Indicator with smooth morph + optional stretch
...
const LiquidGlassNav = () => {
  ...
  // Determine which index to show indicator for
  const showIndicatorForIndex = hoveredIndex !== null ? hoveredIndex : activeIndex;

  // Handle hover with direction tracking for smooth morph
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center">
            ...
            {/* ==================== DESKTOP NAV - ULTRA SMOOTH ==================== */}
            <div className="hidden md:flex items-center">
              {/* Glass Container with layout */}
              <motion.div
                layout
                ...
                style={{
                  ...
                  boxShadow: '0 0 20px rgba(255,255,255,0.05), 0 4px 30px rgba(0,0,0,0.08), inset 0 1px 0 rgba(255,255,255,0.3)',
                }}
              >
                {/* Menu Items - Indicator inside each item */}
                {navLinks.map((link, index) => {
                  ...
                  return (
                    ...
                    >
                      {/* Liquid Indicator with morph + stretch */}
                      {isActive && <LiquidIndicator direction={direction} />}
                      
                      {/* Text with ZOOM - scale 1.08 active, 1.04 hover */}
                      <motion.span
              ...
              <Link
                to="/partisipasi"
                className="ml-4 px-5 py-2.5 rounded-full text-sm font-medium transition-all duration-300 hover:-translate-y-0.5"
                style={{
                ...
                {navLinks.map((link, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        {/* Mobile Liquid Indicator with morph */}
                        {isMobileActive && (
                          <motion.div
                        ...
                        {/* Mobile Text with ZOOM - 1.08 active */}
                        <motion.span
